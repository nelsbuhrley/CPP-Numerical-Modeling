...
def create_plots(data, max_plot_time, driver_frequency, output_filepath):
    ...
    # Calculate Poincaré section sampling
    driver_period = 2 * np.pi / driver_frequency
    poincare_times = np.arange(0, data['Time'].max(), driver_period)

    # Find closest data points to Poincaré sampling times
    poincare_indices = []
    for t_sample in poincare_times:
        idx = (np.abs(data['Time'] - t_sample)).argmin()
        poincare_indices.append(idx)
